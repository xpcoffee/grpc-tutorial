    ...
    public StreamObserver<UserPreferencesRequest> getShortlistedMovies(StreamObserver<UserPreferencesResponse> responseObserver) {
        return new StreamObserver<>() {
            ...
            public void onNext(UserPreferencesRequest request) {
                ...
                if (isEligible()) {
                    System.out.println("Shortlisting movie for user " + request.getUserid());
                    responseObserver.onNext(
                            UserPreferencesResponse.newBuilder().setMovie(request.getMovie()).build()
                    );
                } else {
                    System.out.println("Not shortlisting movie for user " + request.getUserid());
                }
            }
